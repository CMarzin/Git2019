...
rvm.articles.create({content: "Créer le fichier .ruby-version avec le contenu suivant: `ruby-2.3.1`. Ce fichier permet de décrire ",published_at: Time.zone.now})
rvm.articles.create({content: "Créer le fichier .ruby-gemset avec le contenu suivant: `mydoc`. Ce fichier permet de définir ",published_at: Time.zone.now})
rvm.articles.create({content: "Pour rechercher l'environnement il faut faire un `cd .` ou relancer la console.",published_at: Time.zone.now})
